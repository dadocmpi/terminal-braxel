...
const news = [
  { time: '08:30', impact: 'HIGH', event: 'CPI Data Release', currency: 'USD' },
  { time: '10:00', impact: 'HIGH', event: 'Unemployment Claims', currency: 'USD' },
...
export const MarketNews = () => {
  ...
  return (
    <Card className="bg-card/50 border-border/50 p-4 h-full">
      <div className="flex items-center justify-between mb-4">
        <h3 className="text-[10px] font-bold text-muted-foreground uppercase tracking-widest flex items-center gap-2">
          <Globe className="w-3 h-3" /> Institutional News Feed
        </h3>
      ...
      <div className="space-y-3">
        {news.map((item, i) => {
          ...
          return (
            <div 
              key={i} 
              className={`flex items-center gap-3 p-2 rounded bg-secondary/20 border border-border/30 transition-opacity duration-500 ${past ? 'opacity-30 grayscale' : 'opacity-100'}`}
            >
              ...
              {!past && item.impact === 'HIGH' && <AlertCircle className="w-4 h-4 text-bear animate-pulse" />}
            </div>
          );
        })}
      </div>
